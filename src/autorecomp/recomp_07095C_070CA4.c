/* Auto-generated recompiled code for Neo Drift Out */
/* Source range: $07095C - $070CA4 */
/* Functions: 50 */

#include <neogeorecomp/neogeorecomp.h>
#include "recomp_funcs.h"

/* $07095C-$07096C  (4 instructions, 16 bytes) */
void jt_07095C(void) {
    M68K_OR8(g_m68k.d[7], 0xe);
    M68K_OR8(g_m68k.d[7], 0x16);
    M68K_OR8(g_m68k.d[7], 0x58);
    M68K_OR8(g_m68k.d[7], 0x60);
    /* WARNING: function did not end with RTS */
}

/* $07096C-$07097C  (4 instructions, 16 bytes) */
void jt_07096C(void) {
    M68K_OR8(g_m68k.d[7], 0xa2);
    M68K_OR8(g_m68k.d[7], 0xe4);
    M68K_OR8(g_m68k.d[7], 0xec);
    M68K_OR8(g_m68k.d[7], 0x2e);
    /* WARNING: function did not end with RTS */
}

/* $07097C-$07098C  (4 instructions, 16 bytes) */
void jt_07097C(void) {
    M68K_OR8(g_m68k.d[7], 0x36);
    M68K_OR8(g_m68k.d[7], 0x3e);
    M68K_OR8(g_m68k.d[7], 0x80);
    M68K_OR8(g_m68k.d[7], 0x88);
    /* WARNING: function did not end with RTS */
}

/* $07098C-$07099C  (4 instructions, 16 bytes) */
void jt_07098C(void) {
    M68K_OR8(g_m68k.d[7], 0xca);
    M68K_OR8(g_m68k.d[7], 0xc);
    M68K_OR8(g_m68k.d[7], 0x14);
    M68K_OR8(g_m68k.d[7], 0x56);
    /* WARNING: function did not end with RTS */
}

/* $07099C-$0709AC  (4 instructions, 16 bytes) */
void jt_07099C(void) {
    M68K_OR8(g_m68k.d[7], 0x5e);
    M68K_OR8(g_m68k.d[7], 0x72);
    M68K_OR8(g_m68k.d[7], 0x94);
    M68K_OR8(g_m68k.d[7], 0x9c);
    /* WARNING: function did not end with RTS */
}

/* $0709AC-$0709BC  (4 instructions, 16 bytes) */
void jt_0709AC(void) {
    M68K_OR8(g_m68k.d[7], 0xa4);
    M68K_OR8(g_m68k.d[7], 0xac);
    M68K_OR8(g_m68k.d[7], 0xb4);
    M68K_OR8(g_m68k.d[7], 0xbc);
    /* WARNING: function did not end with RTS */
}

/* $0709BC-$0709CC  (4 instructions, 16 bytes) */
void jt_0709BC(void) {
    M68K_OR8(g_m68k.d[7], 0xc4);
    M68K_OR8(g_m68k.d[7], 0xe8);
    M68K_OR8(g_m68k.d[7], 0xfe);
    M68K_OR8(g_m68k.d[7], 0x6);
    /* WARNING: function did not end with RTS */
}

/* $0709CC-$0709DC  (4 instructions, 16 bytes) */
void jt_0709CC(void) {
    M68K_OR8(g_m68k.d[7], 0xe);
    M68K_OR8(g_m68k.d[7], 0x16);
    M68K_OR8(g_m68k.d[7], 0x1e);
    M68K_OR8(g_m68k.d[7], 0x26);
    /* WARNING: function did not end with RTS */
}

/* $0709DC-$0709EC  (4 instructions, 16 bytes) */
void jt_0709DC(void) {
    M68K_OR8(g_m68k.d[7], 0x2e);
    M68K_OR8(g_m68k.d[7], 0x36);
    M68K_OR8(g_m68k.d[7], 0x3e);
    M68K_OR8(g_m68k.d[7], 0x46);
    /* WARNING: function did not end with RTS */
}

/* $0709EC-$0709FC  (4 instructions, 16 bytes) */
void jt_0709EC(void) {
    M68K_OR8(g_m68k.d[7], 0x4e);
    M68K_OR8(g_m68k.d[7], 0x56);
    M68K_OR8(g_m68k.d[7], 0x5e);
    M68K_OR8(g_m68k.d[7], 0x66);
    /* WARNING: function did not end with RTS */
}

/* $0709FC-$070A0C  (4 instructions, 16 bytes) */
void jt_0709FC(void) {
    M68K_OR8(g_m68k.d[7], 0x6e);
    M68K_OR8(g_m68k.d[7], 0x96);
    M68K_OR8(g_m68k.d[7], 0xba);
    M68K_OR8(g_m68k.d[7], 0xc2);
    /* WARNING: function did not end with RTS */
}

/* $070A0C-$070A1C  (4 instructions, 16 bytes) */
void jt_070A0C(void) {
    M68K_OR8(g_m68k.d[7], 0xca);
    M68K_OR8(g_m68k.d[7], 0xe2);
    M68K_OR8(g_m68k.d[7], 0xea);
    M68K_OR8(g_m68k.d[7], 0xf2);
    /* WARNING: function did not end with RTS */
}

/* $070A1C-$070A2C  (4 instructions, 16 bytes) */
void jt_070A1C(void) {
    M68K_OR8(g_m68k.d[7], 0xfa);
    M68K_OR8(g_m68k.d[7], 0x2);
    M68K_OR8(g_m68k.d[7], 0xa);
    M68K_OR8(g_m68k.d[7], 0x12);
    /* WARNING: function did not end with RTS */
}

/* $070A2C-$070A3C  (4 instructions, 16 bytes) */
void jt_070A2C(void) {
    M68K_OR8(g_m68k.d[7], 0x1a);
    M68K_OR8(g_m68k.d[7], 0x22);
    M68K_OR8(g_m68k.d[7], 0x2a);
    M68K_OR8(g_m68k.d[7], 0x40);
    /* WARNING: function did not end with RTS */
}

/* $070A3C-$070A4C  (4 instructions, 16 bytes) */
void jt_070A3C(void) {
    M68K_OR8(g_m68k.d[7], 0x64);
    M68K_OR8(g_m68k.d[7], 0x6c);
    M68K_OR8(g_m68k.d[7], 0x74);
    M68K_OR8(g_m68k.d[7], 0x7c);
    /* WARNING: function did not end with RTS */
}

/* $070A4C-$070A5C  (4 instructions, 16 bytes) */
void jt_070A4C(void) {
    M68K_OR8(g_m68k.d[7], 0x84);
    M68K_OR8(g_m68k.d[7], 0x8c);
    M68K_OR8(g_m68k.d[7], 0x94);
    M68K_OR8(g_m68k.d[7], 0xba);
    /* WARNING: function did not end with RTS */
}

/* $070A5C-$070A6C  (4 instructions, 16 bytes) */
void jt_070A5C(void) {
    M68K_OR8(g_m68k.d[7], 0xce);
    M68K_OR8(g_m68k.d[7], 0xd6);
    M68K_OR8(g_m68k.d[7], 0x26);
    M68K_OR8(g_m68k.d[7], 0x2e);
    /* WARNING: function did not end with RTS */
}

/* $070A6C-$070A7C  (4 instructions, 16 bytes) */
void jt_070A6C(void) {
    M68K_OR8(g_m68k.d[7], 0x90);
    M68K_OR8(g_m68k.d[7], 0xe0);
    M68K_OR8(g_m68k.d[7], 0xe8);
    M68K_OR8(g_m68k.d[7], 0x4a);
    /* WARNING: function did not end with RTS */
}

/* $070A7C-$070A8C  (4 instructions, 16 bytes) */
void jt_070A7C(void) {
    M68K_OR8(g_m68k.d[7], 0x56);
    M68K_OR8(g_m68k.d[7], 0x5e);
    M68K_OR8(g_m68k.d[7], 0xae);
    M68K_OR8(g_m68k.d[7], 0xb6);
    /* WARNING: function did not end with RTS */
}

/* $070A8C-$070A9C  (4 instructions, 16 bytes) */
void jt_070A8C(void) {
    M68K_OR8(g_m68k.d[7], 0x18);
    M68K_OR8(g_m68k.d[7], 0x68);
    M68K_OR8(g_m68k.d[7], 0x70);
    M68K_OR8(g_m68k.d[7], 0xd2);
    /* WARNING: function did not end with RTS */
}

/* $070A9C-$070AAC  (4 instructions, 16 bytes) */
void jt_070A9C(void) {
    M68K_OR8(g_m68k.d[7], 0xda);
    M68K_OR8(g_m68k.d[7], 0xe2);
    M68K_OR8(g_m68k.d[7], 0xea);
    M68K_OR8(g_m68k.d[7], 0x0);
    /* WARNING: function did not end with RTS */
}

/* $070AAC-$070ABC  (4 instructions, 16 bytes) */
void jt_070AAC(void) {
    M68K_OR8(g_m68k.d[7], 0x28);
    M68K_OR8(g_m68k.d[7], 0x30);
    M68K_OR8(g_m68k.d[7], 0x38);
    M68K_OR8(g_m68k.d[7], 0x40);
    /* WARNING: function did not end with RTS */
}

/* $070ABC-$070ACC  (4 instructions, 16 bytes) */
void jt_070ABC(void) {
    M68K_OR8(g_m68k.d[7], 0x48);
    M68K_OR8(g_m68k.d[7], 0x50);
    M68K_OR8(g_m68k.d[7], 0x58);
    M68K_OR8(g_m68k.d[7], 0x7a);
    /* WARNING: function did not end with RTS */
}

/* $070ACC-$070ADC  (4 instructions, 16 bytes) */
void jt_070ACC(void) {
    M68K_OR8(g_m68k.d[7], 0x8e);
    M68K_OR8(g_m68k.d[7], 0x96);
    M68K_OR8(g_m68k.d[7], 0x9e);
    M68K_OR8(g_m68k.d[7], 0xc2);
    /* WARNING: function did not end with RTS */
}

/* $070ADC-$070AEC  (4 instructions, 16 bytes) */
void jt_070ADC(void) {
    M68K_OR8(g_m68k.d[7], 0xee);
    M68K_OR8(g_m68k.d[7], 0xf6);
    M68K_OR8(g_m68k.d[7], 0xfe);
    M68K_OR8(g_m68k.d[7], 0x6);
    /* WARNING: function did not end with RTS */
}

/* $070AEC-$070AFC  (4 instructions, 16 bytes) */
void jt_070AEC(void) {
    M68K_OR8(g_m68k.d[7], 0xe);
    M68K_OR8(g_m68k.d[7], 0x16);
    M68K_OR8(g_m68k.d[7], 0x1e);
    M68K_OR8(g_m68k.d[7], 0x42);
    /* WARNING: function did not end with RTS */
}

/* $070AFC-$070B0C  (4 instructions, 16 bytes) */
void jt_070AFC(void) {
    M68K_OR8(g_m68k.d[7], 0x60);
    M68K_OR8(g_m68k.d[7], 0x68);
    M68K_OR8(g_m68k.d[7], 0x70);
    M68K_OR8(g_m68k.d[7], 0xb0);
    /* WARNING: function did not end with RTS */
}

/* $070B0C-$070B1C  (4 instructions, 16 bytes) */
void jt_070B0C(void) {
    M68K_OR8(g_m68k.d[7], 0xb8);
    M68K_OR8(g_m68k.d[7], 0xc0);
    M68K_OR8(g_m68k.d[7], 0xc8);
    M68K_OR8(g_m68k.d[7], 0xe4);
    /* WARNING: function did not end with RTS */
}

/* $070B1C-$070B2C  (4 instructions, 16 bytes) */
void jt_070B1C(void) {
    M68K_OR8(g_m68k.d[7], 0xf8);
    M68K_OR8(g_m68k.d[7], 0x0);
    M68K_OR8(g_m68k.d[7], 0x8);
    M68K_OR8(g_m68k.d[7], 0x10);
    /* WARNING: function did not end with RTS */
}

/* $070B2C-$070B3C  (4 instructions, 16 bytes) */
void jt_070B2C(void) {
    M68K_OR8(g_m68k.d[7], 0x18);
    M68K_OR8(g_m68k.d[7], 0x2c);
    M68K_OR8(g_m68k.d[7], 0x4e);
    M68K_OR8(g_m68k.d[7], 0x56);
    /* WARNING: function did not end with RTS */
}

/* $070B3C-$070B4C  (4 instructions, 16 bytes) */
void jt_070B3C(void) {
    M68K_OR8(g_m68k.d[7], 0x5e);
    M68K_OR8(g_m68k.d[7], 0x66);
    M68K_OR8(g_m68k.d[7], 0x6e);
    M68K_OR8(g_m68k.d[7], 0x76);
    /* WARNING: function did not end with RTS */
}

/* $070B4C-$070B5C  (4 instructions, 16 bytes) */
void jt_070B4C(void) {
    M68K_OR8(g_m68k.d[7], 0x7e);
    M68K_OR8(g_m68k.d[7], 0xa2);
    M68K_OR8(g_m68k.d[7], 0xb8);
    M68K_OR8(g_m68k.d[7], 0xc0);
    /* WARNING: function did not end with RTS */
}

/* $070B5C-$070B6C  (4 instructions, 16 bytes) */
void jt_070B5C(void) {
    M68K_OR8(g_m68k.d[7], 0xc8);
    M68K_OR8(g_m68k.d[7], 0xd0);
    M68K_OR8(g_m68k.d[7], 0xd8);
    M68K_OR8(g_m68k.d[7], 0x1a);
    /* WARNING: function did not end with RTS */
}

/* $070B6C-$070B7C  (4 instructions, 16 bytes) */
void jt_070B6C(void) {
    M68K_OR8(g_m68k.d[7], 0x5c);
    M68K_OR8(g_m68k.d[7], 0x9e);
    M68K_OR8(g_m68k.d[7], 0xe0);
    M68K_OR8(g_m68k.d[7], 0xe8);
    /* WARNING: function did not end with RTS */
}

/* $070B7C-$070B8C  (4 instructions, 16 bytes) */
void jt_070B7C(void) {
    M68K_OR8(g_m68k.d[7], 0xf0);
    M68K_OR8(g_m68k.d[7], 0xf8);
    M68K_OR8(g_m68k.d[7], 0x0);
    M68K_OR8(g_m68k.d[7], 0x42);
    /* WARNING: function did not end with RTS */
}

/* $070B8C-$070B9C  (4 instructions, 16 bytes) */
void jt_070B8C(void) {
    M68K_OR8(g_m68k.d[7], 0x84);
    M68K_OR8(g_m68k.d[7], 0xc6);
    M68K_OR8(g_m68k.d[7], 0x8);
    M68K_OR8(g_m68k.d[7], 0x10);
    /* WARNING: function did not end with RTS */
}

/* $070B9C-$070BAC  (4 instructions, 16 bytes) */
void jt_070B9C(void) {
    M68K_OR8(g_m68k.d[7], 0x18);
    M68K_OR8(g_m68k.d[7], 0x2c);
    M68K_OR8(g_m68k.d[7], 0x4e);
    M68K_OR8(g_m68k.d[7], 0x56);
    /* WARNING: function did not end with RTS */
}

/* $070BAC-$070BBC  (4 instructions, 16 bytes) */
void jt_070BAC(void) {
    M68K_OR8(g_m68k.d[7], 0x5e);
    M68K_OR8(g_m68k.d[7], 0x66);
    M68K_OR8(g_m68k.d[7], 0x6e);
    M68K_OR8(g_m68k.d[7], 0x76);
    /* WARNING: function did not end with RTS */
}

/* $070BBC-$070BCC  (4 instructions, 16 bytes) */
void jt_070BBC(void) {
    M68K_OR8(g_m68k.d[7], 0x7e);
    M68K_OR8(g_m68k.d[7], 0xa2);
    M68K_OR8(g_m68k.d[7], 0xb8);
    M68K_OR8(g_m68k.d[7], 0xc0);
    /* WARNING: function did not end with RTS */
}

/* $070BCC-$070BDC  (4 instructions, 16 bytes) */
void jt_070BCC(void) {
    M68K_OR8(g_m68k.d[7], 0xc8);
    M68K_OR8(g_m68k.d[7], 0xd2);
    M68K_OR8(g_m68k.d[7], 0xdc);
    M68K_OR8(g_m68k.d[7], 0xe4);
    /* WARNING: function did not end with RTS */
}

/* $070BDC-$070BEC  (4 instructions, 16 bytes) */
void jt_070BDC(void) {
    M68K_OR8(g_m68k.d[7], 0xec);
    M68K_OR8(g_m68k.d[7], 0xf6);
    M68K_OR8(g_m68k.d[7], 0x0);
    M68K_OR8(g_m68k.d[7], 0x8);
    /* WARNING: function did not end with RTS */
}

/* $070BEC-$070BFC  (4 instructions, 16 bytes) */
void jt_070BEC(void) {
    M68K_OR8(g_m68k.d[7], 0x10);
    M68K_OR8(g_m68k.d[7], 0x1a);
    M68K_OR8(g_m68k.d[7], 0x24);
    M68K_OR8(g_m68k.d[7], 0x2c);
    /* WARNING: function did not end with RTS */
}

/* $070BFC-$070C0C  (4 instructions, 16 bytes) */
void jt_070BFC(void) {
    M68K_OR8(g_m68k.d[7], 0x34);
    M68K_OR8(g_m68k.d[7], 0x74);
    M68K_OR8(g_m68k.d[7], 0x7c);
    M68K_OR8(g_m68k.d[7], 0x84);
    /* WARNING: function did not end with RTS */
}

/* $070C0C-$070C1C  (4 instructions, 16 bytes) */
void jt_070C0C(void) {
    M68K_OR8(g_m68k.d[7], 0x8c);
    M68K_OR8(g_m68k.d[7], 0xa4);
    M68K_OR8(g_m68k.d[7], 0xae);
    M68K_OR8(g_m68k.d[7], 0xb6);
    /* WARNING: function did not end with RTS */
}

/* $070C1C-$070C2C  (4 instructions, 16 bytes) */
void jt_070C1C(void) {
    M68K_OR8(g_m68k.d[7], 0xbe);
    M68K_OR8(g_m68k.d[7], 0xc6);
    M68K_OR8(g_m68k.d[7], 0xce);
    M68K_OR8(g_m68k.d[7], 0xd6);
    /* WARNING: function did not end with RTS */
}

/* $070C5C-$070C60  (2 instructions, 4 bytes) */
void jt_070C5C(void) {
    M68K_ASR16(g_m68k.d[0], 0x8);
    /* TODO $070C5E: dc.w $ff20  [FF 20] */
    /* WARNING: function did not end with RTS */
}

/* $070C64-$070C68  (2 instructions, 4 bytes) */
void jt_070C64(void) {
    M68K_ASR16(g_m68k.d[0], 0x8);
    /* TODO $070C66: dc.w $ff20  [FF 20] */
    /* WARNING: function did not end with RTS */
}

/* $070C8C-$070C94  (4 instructions, 8 bytes) */
void jt_070C8C(void) {
    /* TODO $070C8C: dc.w $ff16  [FF 16] */
    /* TODO $070C8E: dc.w $ff16  [FF 16] */
    M68K_OR8(g_m68k.d[1], bus_read8(g_m68k.a[6]));
    M68K_OR8(g_m68k.d[0], g_m68k.d[0]);
    /* WARNING: function did not end with RTS */
}

/* $070C94-$070C9C  (4 instructions, 8 bytes) */
void jt_070C94(void) {
    /* TODO $070C94: dc.w $ff16  [FF 16] */
    /* TODO $070C96: dc.w $ff16  [FF 16] */
    M68K_OR8(g_m68k.d[1], bus_read8(g_m68k.a[6]));
    M68K_OR8(g_m68k.d[0], g_m68k.d[0]);
    /* WARNING: function did not end with RTS */
}

/* $070C9C-$070CA4  (4 instructions, 8 bytes) */
void jt_070C9C(void) {
    /* TODO $070C9C: dc.w $ff16  [FF 16] */
    /* TODO $070C9E: dc.w $ff16  [FF 16] */
    M68K_OR8(g_m68k.d[1], bus_read8(g_m68k.a[6]));
    M68K_OR8(g_m68k.d[0], g_m68k.d[0]);
    /* WARNING: function did not end with RTS */
}
